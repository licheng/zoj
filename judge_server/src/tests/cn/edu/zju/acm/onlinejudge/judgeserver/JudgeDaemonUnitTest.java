...
public class JudgeDaemonUnitTest {
    private JudgeDaemon daemon;
	...
	daemon = new JudgeDaemon(this, queue, InetAddress.getLocalHost(), server.getPort());
	daemon.start();
	...
	submission = DAOFactory.getSubmissionDAO().getSubmission(0);
	assertEquals(JudgeReply.ACCEPTED, submission.getJudgeReply());
	assertEquals(1100, submission.getTimeConsumption());
	assertEquals(2, submission.getMemoryConsumption());
    }
	...
	daemon.join();
	synchronized (this) {
	    daemon = new JudgeDaemon(this, queue, InetAddress.getLocalHost(), server.getPort() + 1);
	    daemon.start();
	    queue.push(submission);
	    wait();
	}
